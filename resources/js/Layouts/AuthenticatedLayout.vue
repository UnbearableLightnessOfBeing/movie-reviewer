...
const removeEventLestener = Inertia.on("before", (e) => {
    removeEventLestener();
    animate.value = true;
    e.preventDefault();

    console.log("animating");
    console.log("navigation stopped", e.detail.visit.url);

    setTimeout(() => {
        animate.value = false;
        console.log("stop animation");
        Inertia.get(e.detail.visit.url);
    }, 650);
});
        ...
        <div class="min-h-screen bg-primary">
                            ...
                            >
                                ...
                                <NavLink
                                    v-if="
                                        ...
                                        $page.props.auth.user.roles.map(
                                    ...
                                    <template #trigger>
                                        <span class="inline-flex rounded-md">
                                            <img
                                                ...
                                                "
                                                width="40"
                                                class="mr-2 rounded-full overflow-hedden"
                                                alt="avatar"
                                            />
                                            <button
                                                type="button"
                            ...
                            <button
                                @click="
                                    showingNavigationDropdown =
                                        !showingNavigationDropdown
                                "
                                class="inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-secondary hover:bg-primarylight focus:outline-none focus:bg-primarylight focus:text-secindary transition duration-150 ease-in-out"
                            >
                                <svg
                                    class="h-6 w-6"
                                    stroke="currentColor"
                                ...
                                :src="
                                    $page.props.auth.user.avatar
                                        ? $page.props.ziggy.url +
                                          '/storage/' +
                                          $page.props.auth.user.avatar
                                        : getAvatarSource(user.name)
                                "
                                width="40"
                                class="mr-2 mx-4 rounded-full overflow-hedden"
                                alt="avatar"
                            />
                            <ResponsiveNavLink
                                v-if="$page.props.auth.user"
            ...
            <div class="w-full">
                <transition name="test" mode="out-in" appear>
                    <div
                        v-if="shown"
                        class="w-[300px] h-[100px] bg-red-400"
                    ></div>
                    <div v-else class="w-[300px] h-[100px] bg-indigo-400"></div>
                </transition>
                <button @click="shown = !shown" class="border p-4 bg-green-400">
                    show
                </button>
            </div>

            <!-- Page Content -->
            <transition name="page" mode="in-out" appear>
                <div :key="$page.url" v-if="animate">
                    <slot />
                </div>
            </transition>
        </div>
